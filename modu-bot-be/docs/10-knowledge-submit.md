...
제출 즉시 3 HS 토큰이 지급되며, 관리자 승인 시 추가로 10 HS 토큰이 지급됩니다.
...
| questionId | String | O    | 답변 대상 질문의 UUID (`/questions/next`에서 획득) |
...
4. `questionId`로 질문 텍스트를 조회하여 `originalQuestion` 필드에 자동 저장 (클라이언트가 직접 전달하지 않음)
5. 제출 즉시 제출자 지갑으로 3 HS 토큰 자동 지급 (비동기, 실패 시 제출 결과에 영향 없음)
6. 승인 후 제출자 지갑으로 추가 10 HS 토큰 자동 지급
